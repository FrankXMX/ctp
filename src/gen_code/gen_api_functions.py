...
class ApiGenerator:
    ...
    def process_line(self, line: str):
        ...
        if "virtual void On" in line:
            ...
            self.callbackslines.append(line)
        elif "virtual int Req" in line:
            self.process_function(line)
            # self.reqfunction.append(line)
    ...
    def generate_header_task(self):
        """"""
        filename = f"{self.prefix}_{self.name}_header_task.h"
        with open(filename, "w") as f:
            for func in self.callbackslines:
                line = f"{func};\n\n"
                f.write(line)
